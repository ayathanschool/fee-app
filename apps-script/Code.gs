...
function doGet(e) {
  try {
    ...
    if (action === 'getDueNotifications') {
      const classFilter = params.class || '';
      const threshold = params.threshold || 0;
      return jsonResponse(getDueNotifications(classFilter, threshold));
    }
...
/**
 * Get due fee notifications for students
 * This function identifies students with pending fees and generates notifications
 * @param {string} classFilter - Optional class filter
 * @param {number} daysThreshold - Optional days threshold for due dates
 * @return {Object} Notification data for students with pending fees
 */
function getDueNotifications(classFilter = '', daysThreshold = 0) {
  // Get all students, filtered by class if provided
  const students = getSheetData(CONFIG.SHEET_NAMES.STUDENTS).data
    .filter(s => !classFilter || String(s.class || s.cls).trim().toLowerCase() === String(classFilter).trim().toLowerCase());
  ...
  if (!students.length) {
    return { 
      ...
      totalStudents: 0,
      totalPending: 0,
      message: classFilter ? `No students found in class ${classFilter}` : 'No students found'
    };
  }
  
  // Get all fee heads
  const allFeeHeads = getSheetData(CONFIG.SHEET_NAMES.FEEHEADS).data
    .map(normalizeFeeHeadRecord);
  
  // Get transactions sheet for fee payment checks
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const txSheet = ss.getSheetByName(CONFIG.SHEET_NAMES.TRANSACTIONS);
  
  if (!txSheet) {
    return { ok: false, error: 'transactions_sheet_not_found' };
  }
  ...
  for (const student of students) {
    ...
    const studentClass = student.class || student.cls;
    const studentName = student.name;
    
    if (!admNo || !studentClass) continue;
    
    // Get applicable fee heads for this student
    const applicableFeeHeads = allFeeHeads.filter(f => 
      String(f.class).trim().toLowerCase() === String(studentClass).trim().toLowerCase()
    );
    
    // Track pending fees for this student
    const pendingFees = [];
    let totalPending = 0;
    
    // Check each fee head
    for (const feeHead of applicableFeeHeads) {
      // Skip if already paid
      if (isFeePaid(txSheet, admNo, feeHead.feeHead)) {
        continue;
      }
      
      // Check if due date is approaching or passed
      const isDue = true; // By default, all unpaid fees are due
      let daysOverdue = 0;
      
      if (feeHead.dueDate) {
        const dueDate = new Date(feeHead.dueDate);
        if (!isNaN(dueDate)) {
          const timeDiff = today.getTime() - dueDate.getTime();
          daysOverdue = Math.floor(timeDiff / (1000 * 3600 * 24));
          
          // Only include if past due date or within threshold
      ...
      pendingFees.push({
        ...
        amount: Number(feeHead.amount) || 0,
        dueDate: feeHead.dueDate,
        daysOverdue: Math.max(0, daysOverdue),
        isOverdue: daysOverdue > 0
      });
      
      totalPending += Number(feeHead.amount) || 0;
    }
    
    // Only add students with pending fees
    if (pendingFees.length > 0) {
      notifications.push({
        ...
        urgency: pendingFees.some(f => f.isOverdue) ? 'high' : 'medium'
      });
    }
  }
  
  // Sort notifications by urgency and total amount
  notifications.sort((a, b) => {
    // First by urgency (high first)
    if (a.urgency === 'high' && b.urgency !== 'high') return -1;
    if (a.urgency !== 'high' && b.urgency === 'high') return 1;
    
    // Then by total pending amount (highest first)
  ...
  return {
    ...
    summary: {
      highUrgency: notifications.filter(n => n.urgency === 'high').length,
      mediumUrgency: notifications.filter(n => n.urgency === 'medium').length,
      totalDueAmount: notifications.reduce((sum, n) => sum + n.totalPending, 0)
    }
  };
}
